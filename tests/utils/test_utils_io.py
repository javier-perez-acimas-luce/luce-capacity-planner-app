...
def test_load_config_raises_exception(root_dir):
    with pytest.raises(FileNotFoundError):
        io.load_config(root_dir + "fake_file")


def test_load_config(root_dir):
    result = io.load_config(root_dir + "data/config.yaml")
...
def test_fetch_env_variable_raises_error_when_key_error(config):
    with pytest.raises(KeyError):
...
def test_get_date_sub_path_returns_correct_path():
